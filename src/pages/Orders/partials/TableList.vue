...
import { formatDate, formatCurrency } from "@/utils/_S9/helper"
...
const confirmDeleteRow = (item: OrderInterfaces.Order) => {
	confirm.require({
...
if (props.permissions.read)
	menuModel.value.push({
		...
		command: () =>
			router.push({
				name: "order",
				params: { orderId: selectedRow.value.id },
			}),
	})
		...
		<div class="intro-y col-span-12">
			<div class="box overflow-hidden">
				...
				<DataTable
					...
					data-key="id"
					size="small"
					:global-filter-fields="['created_at', 'total', 'order_number']"
					context-menu
				...
				>
					...
					<Column
						v-if="!hideColumns.includes('order')"
						sortable
						field="order_number"
						header="Order #"
						class="pl-4"
					...
					<Column v-if="!hideColumns.includes('customer1')" header="Customer">
						<template #body="{ data }">
